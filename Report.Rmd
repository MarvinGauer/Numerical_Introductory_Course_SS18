...
Two quite old but still very well written papers are [@Lyes] and [@Kuncir]. Both deal with the adaptive simpson quadrature elaborated in [Section 3.2.3.](#sec:bib) and are especially informative for beginners on the topic.
...
One example of an adaptive algorithm would be the adaptive simpson quadrature. There are several versions of this adaptive routine, but the one I want to elaborate on is the one published by @Lyes. Below you can find the pseudocode for a limited function $f:[a;b]\rightarrow \mathbb{R}$ and an error threshold $\varepsilon$. \newpage

\begin{shaded*}
AdaptiveSimpson($f$, $[a;b]$, $\varepsilon$, $I \approx \int_a^bf(x)dx$)
\begin{itemize}
...
\item[] Else
\begin{itemize}
\item[] Return AdaptiveSimpson($f$, $[a;m]$, $\frac{\varepsilon}{2}$,$I_1$) + AdaptiveSimpson($f$, $[m;b]$, $\frac{\varepsilon}{2}$, $I_2$)
\end{itemize}
\end{itemize}
\end{shaded*}
